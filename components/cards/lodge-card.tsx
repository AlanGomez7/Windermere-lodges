...
export default function LodgeCard({ lodge, needsButton }: any) {
  ...
  const visibleItems = showAll ? lodge.features : lodge.features.slice(0, 6);
  ...
  return (
    <Link href={`/our-lodges/${lodge.refNo}`} key={lodge.id} prefetch>
      ...
      >
        ...
        <CardContent className="p-5">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {visibleItems.map((feature: string, index: number) => (
              <Badge key={index} variant="outline" className="bg-gray-50 font-light">
                {feature}
              </Badge>
            ))}
            {remainingCount > 0 && (
        ...
        {needsButton&&<CardFooter className="p-5 flex justify-between items-center">
          <div>
